...
## 2026-09-21 · 混合检索的 RRF 实现约定（每路取 10 条 / key 用 documentId#chunkIndex / RRF_K=60）

- **背景**：W5-② 要把 BM25（关键词）与 kNN（向量）两路结果融合成一个排序。两路分数不在一个量纲上——BM25 无上界、余弦被 ES 换算成 0~1，直接加权求和等于让 BM25 一家说了算。
- **决策**：在 Java 里手工实现 RRF，具体约定：
  - **每路各取 10 条候选**（`FUSION_CANDIDATES = 10`）再做融合。取太窄会让"单榜靠后、另一榜靠前"的块根本没机会进融合池——而它恰恰是混合检索最该救回来的东西。
  - **融合 key 用 `documentId + "#" + chunkIndex`**（`RetrievedChunk` 里现成有这两个字段，能唯一标识一个块）。不用 `content` 当 key（内容可能重复）。
  - **平滑常数 `RRF_K = 60`**（RRF 原论文经验值）。抽成常量，方便 W12 做 k=10/60/100 的对比实验。
  - **`RetrievedChunk.score` 的语义随模式变化**：keyword → BM25 分；vector → 余弦换算分（0~1）；hybrid → **RRF 融合分（0.0x 量级）**。这不是 bug，已写进代码注释与接口说明。
- **放弃的方案**：
  - 两路分数加权求和（量纲不同，等于只听了一个评委的）
  - 先各自归一化再加权（每次查询的分数分布不同，固定归一化公式会失真）
  - ES 8.8+ 原生 `retriever.rrf`（融合过程被黑盒吃掉，讲不出原理，面试价值低）
- **后续验证**：W12 在评测集上对比 keyword / vector / hybrid 三路的 recall@3；并验证 RRF 的 k 值（10 / 60 / 100）对结果的影响是否在 2% 以内（若如此，则沿用论文默认 60）。

## 2026-09-21 · 检索失败时降级为"无资料回答"，不让 AI 链路拖垮对话

- **背景**：W5-③ 起，"发消息"这条链路里多了"检索知识库"这一步。ES 是独立进程、还要过 SSH 隧道，故障概率明显高于 MySQL（本项目已踩过一次隧道断开导致 `Connection refused`）。
- **决策**：检索用 try-catch 包住，失败时把参考资料置空、**照常调用模型回答**（prompt 里填"（本次没有检索到相关资料）"，模型会按规则②说"没查到 + 建议转人工"）。**用户完全感知不到检索挂了。**
- **放弃的方案**：检索失败就向前端抛错 / 返回 500 —— 访客会看到"客服连话都不会说了"，等于把**"增强功能"的故障升级成了"核心功能"的故障**。
- **后续**：W10 拆微服务时把这个思路放大成 Sentinel 熔断降级（ai-service 挂了 → 直接排队转人工）。
